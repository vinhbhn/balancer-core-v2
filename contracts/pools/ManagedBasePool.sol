 ...
 * of registered tokens, optional Asset Managers, an admin-controlled swap fee percentage, and an emergency pause
 * mechanism.
 *
...
abstract contract ManagedBasePool is IBasePool, BasePoolAuthorization, BalancerPoolToken, TemporarilyPausable {
        ...
        TemporarilyPausable(pauseWindowDuration, bufferPeriodDuration)
    {
        ...
        _require(tokenLen >= _MIN_TOKENS, Errors.MIN_TOKENS);
        _require(tokenLen <= _MAX_TOKENS, Errors.MAX_TOKENS);
        ...
        // registerTokens checks that tokens and assetManagers are the same length
        vault.registerTokens(poolId, tokens, assetManagers);

        // Set immutable state variables - these cannot be read from during construction
        _vault = vault;
        _poolId = poolId;
        _totalTokens = tokenLen;

        // Immutable variables cannot be initialized inside an if statement, so we must do conditional assignments
        _token0 = tokenLen > 0 ? tokens[0] : IERC20(0);
        _token1 = tokenLen > 1 ? tokens[1] : IERC20(0);
        _token2 = tokenLen > 2 ? tokens[2] : IERC20(0);
        ...
        _scalingFactor2 = tokenLen > 2 ? _computeScalingFactor(tokens[2]) : 0;
    ...
    function _ensureVault(bytes32 poolId) private view {
    ...
    function onJoinPool(
        ...
        if (totalSupply() == 0) {
            ...
            uint256 mintAmount = bptAmountOut - _MINIMUM_BPT;

            _require(mintAmount >= 0, Errors.MINIMUM_BPT);
