...
import { RawManagedWeightedPoolDeployment } from '../../helpers/models/pools/weighted-managed/types';
...
describe('ManagedWeightedPool', function () {
  ...
  context('for a 1 token pool', () => {
    it('reverts if there is a single token', async () => {
      ...
      await expect(ManagedWeightedPool.create({ tokens, weights })).to.be.revertedWith('MIN_TOKENS');
    });
  });
  ...
  function itBehavesAsWeightedPool(numberOfTokens: number) {
    let pool: ManagedWeightedPool, tokens: TokenList;
